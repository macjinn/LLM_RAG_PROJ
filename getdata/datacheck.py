...
params ={'serviceKey' : "pouv9PVQTMTrZSt0KQplGtJlJnHTu9xoMA4E/5g78JpWxC2JfPiBf87rNaAxu6RkT6wCb0+MHwUcEdtKW+biNg==", 
         'pageNo' : '1', 
         'numOfRows' : '10',
         'sBseDt' : '20210730', 
         'eBseDt' : '20210730' }
...
if result_code is not None and result_code.text != '0':
    ...
    if not items:
        ...
        for i, item in enumerate(items, 1):
            name =  item.findtext('cadTpTcNm') 
            institute =  item.findtext('prdNm') #카드상품명
            prd = item.findtext('prdOtl')
            print(f"{i}. 카드유형: {name} / 카드상품명: {institute} / 요약설명: {prd}")